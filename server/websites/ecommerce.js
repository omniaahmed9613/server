...
async function jumia (link)  {
  ...
  let curr;
   try {
       await page.waitForSelector('.-fs0.-pls.-prl h1')
       name=await page.$eval('.-fs0.-pls.-prl h1',name=>name.textContent);    
   }
   catch (error) {
   }
   try {
       await page.waitForSelector('.-b.-ltr.-tal.-fs24')
     ...
     try{
       await page.waitForSelector('.-fw.-fh')
...
async function souq (link)  {
  ...
  let ImgSrc ;
   ...
   try {
      await page.waitForSelector('.small-12.columns.product-title h1');
      name=await page.$eval('.small-12.columns.product-title h1',name=>name.textContent)
   }
   catch (error) {
   }
   try {
       await page.waitForSelector('.price.is.sk-clr1')
     ...
     try{
       await page.waitForSelector('.img-bucket img')
